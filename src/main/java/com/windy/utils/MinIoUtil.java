...
public class MinIoUtil {
    private static Iterator<Result<Item>> iterator;
...
//    public static Integer num;
     ...
     */
    @SneakyThrows(Exception.class)
    public static String upload(String bucketName, File file) throws Exception {
        final InputStream is = new FileInputStream(file);
        final String fileName = file.getName(); //file.getOriginalFilename();
        minioClient.putObject(bucketName, fileName, is, new PutObjectOptions(is.available(), -1));
        is.close();
        return getFileUrl(bucketName, fileName);
    }
...
//                    System.out.println(getFileUrl(bucketName,item.objectName()));
    ...
    public static Integer getIterator(String bucketName) throws  Exception{
        int count=0;
    ...
    public static ArrayList<FileInfos> getFiles(String bucketName, Integer pageSize,
                                                Integer i,
                                                Integer currentPage,
        ...
        try {
            ...
            if (isExist) {
                Iterable<Result<Item>> allFiles = minioClient.listObjects(bucketName);
                iterator= allFiles.iterator();
                count = Iterators.size(iterator);//元素个数
                ...
                for (Result<Item> fileItem : allFiles) {
                    ...
                    num+=1;
                    System.out.println("url="+getFileUrl(bucketName,filename));
//                    System.out.println("url="+ URLDecoder.decode(getFileUrl(bucketName,filename)));
                    if(num>=pageSize)
                        break;
                }
            }
        ...
        try {
            ...
            if (isExist) {
                ...
                for (Result<Item> fileItem : allFiles) {
                    Item item = fileItem.get();
                    //对中文进行编码解码
                    // 中文出现%E6%96%87%E6%A1%A3乱码：URLDecoder.decode进行解码
                    String filename = URLDecoder.decode(item.objectName(),"UTF-8");
                    FileInfos fileInfoIem = new FileInfos(item.lastModified(),
                            filename,
                            getSize(item.size()));
